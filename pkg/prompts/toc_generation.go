...
func GetLanguageInstructionForTOC(langCode string) string {
	switch langCode {
	case "zh":
...
const tocGenerationCommonRules = `CRITICAL PAGE NUMBER EXTRACTION - HIGHEST PRIORITY:
...
func TOCInitPrompt(languageInstruction, content string) string {
...
Document content:
%s`, languageInstruction, tocGenerationCommonRules, content)
}

const tocContinueCommonRules = `CRITICAL REQUIREMENTS - MUST FOLLOW:
...
Return ONLY new sections. If all sections are already in Existing TOC, return an empty array [].`

func TOCContinuePromptWithExisting(languageInstruction, existingJSON, content string) string {
	return fmt.Sprintf(`%s
...
%s`, languageInstruction, existingJSON, content, tocContinueCommonRules)
}
