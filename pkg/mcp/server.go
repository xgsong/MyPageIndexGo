...
func registerTools(s *server.MCPServer) {
	...
	s.AddTool(
		mcp.NewTool(
			...
			mcp.WithString("existing_index_path",
				mcp.Required(),
				mcp.Description("现有索引文件路径"),
			),
			mcp.WithString("new_file_path",
				mcp.Required(),
				mcp.Description("新文档文件路径（PDF 或 Markdown）"),
			),
			mcp.WithString("output_path",
				mcp.Description("输出索引文件路径，默认为 {existing_index_path}.merged.json"),
			),
			mcp.WithString("model",
				mcp.Description("可选的 LLM 模型，覆盖 config.yaml 配置"),
			),
			mcp.WithNumber("max_concurrency",
				mcp.Description("可选的最大并发 LLM 调用数"),
			),
		),
		updateIndexHandler,
	)
